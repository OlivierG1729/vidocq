...
class IndexManager:
    ...
    def _build_indexes(self, documents: Dict[str, str], document_hashes: Dict[str, str]) -> IndexData:
        ...
        embeddings = self.model.encode(
            [clean_documents[name] for name in doc_order],
    ...
    def _save_to_disk(self, index_data: IndexData) -> None:
        ...
        INVERTED_INDEX_FILE.write_text(
            json.dumps(index_data.inverted_index, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
    ...
    def _load_from_disk(self) -> IndexData:
        ...
        inverted_index = (
            json.loads(INVERTED_INDEX_FILE.read_text(encoding="utf-8")) if INVERTED_INDEX_FILE.exists() else {}
        )
        ...
        tfidf_matrix = (
            sparse.load_npz(TFIDF_MATRIX_FILE) if TFIDF_MATRIX_FILE.exists() else sparse.csr_matrix((0, 0))
        )
